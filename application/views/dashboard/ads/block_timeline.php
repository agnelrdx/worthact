...
<div title="<?= $block_global->link ?>" class="col-sm-12 timeline_adv adv_video timeline_post <?= ($block_global->link != '' && $block_global->video == '')? 'adv_link' : '' ?>" data-href="<?= $block_global->link; ?>">
    <div class="inner shadow">
        ...
        <div class="timeline_post_title">
            <h4><?= ucfirst($block_global->heading); ?></h4>
        </div>
        ...
        <?php endif; if($block_global->video != '' && $block_global->link != ''): ?><a class="adv_anchor" data-ripple href="<?= $block_global->link ?>" target="_blank"><?= ($block_global->action_text == '')? 'Know More' : $block_global->action_text; ?></a><?php endif; ?>
    </div>
</div>
<?php endif; if($block_local != 'blah') : ?>
<div title="<?= $block_local->link ?>" class="col-sm-12 timeline_adv adv_video timeline_post <?= ($block_global->link != '' && $block_local->video == '')? 'adv_link' : '' ?>" data-href="<?= $block_local->link; ?>">
    <div class="inner shadow">
        <div class="timeline_post_head">
            <a href="<?= base_url('dashboard/profile/'.$user->main_id) ?>"><img class="shadow" src="<?= base_url(($user->propic != '') ? "assets/userdata/dashboard/propic/".$user->propic : "assets/img/user_placeholder.png"); ?>" alt="<?= ($user->type_id == 1) ? ucfirst($user->firstname).' '.ucfirst($user->lastname) : ucfirst($user->name); ?>" /></a>
            <a class="title" href="<?= base_url('dashboard/profile/'.$user->main_id) ?>"><h5 class="trans"><?= ($user->type_id == 1) ? ucfirst($user->firstname).' '.ucfirst($user->lastname) : ucfirst($user->name); ?></h5></a>
        ...
        <div class="timeline_post_title">
            <h4><?= ucfirst($block_local->heading); ?></h4>
        </div>
        <div class="timeline_post_media">
        ...
        <?php endif; if($block_local->video != '' && $block_local->link != ''): ?><a class="adv_anchor" data-ripple href="<?= $block_local->link ?>" target="_blank"><?= ($block_local->action_text == '')? 'Know More' : $block_local->action_text; ?></a><?php endif; ?>
    </div>
</div>
<?php endif; ?>
